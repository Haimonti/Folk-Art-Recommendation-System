...
def build_user_pos_dict(data):
    """
    Build user -> set of positive items (VERY small memory footprint)
    """
    return data.groupby('user_id')['item_id'].apply(set).to_dict()


def sample_negatives(user_list, user_pos, item_num, neg_num=100):
    """
    Generate negatives on demand (DGSR-style)
    """
    neg = np.zeros((len(user_list), neg_num), dtype=np.int32)

    for i, u in enumerate(user_list):
        pos_items = user_pos.get(u, set())

        j = 0
        while j < neg_num:
            item = np.random.randint(0, item_num)
...
def collate_test(data, user_neg):
    ...
    return torch.Tensor(user_alis).long(), dgl.batch_hetero(graph), torch.Tensor(last_item).long(), \
           torch.Tensor(label).long(), torch.Tensor(length).long(), torch.Tensor(sample_negatives(user, user_pos, item_num)).long()
