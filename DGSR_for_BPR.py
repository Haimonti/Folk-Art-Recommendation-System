...
class DGSR(nn.Module):
    ...
    def forward(self, g, user_index=None, last_item_index=None, pos_tar=None, neg_tar=None, is_training=False):
        feat_dict = None
        user_layer = []
        device = g.device # automatically gets the device the graph is on
        ...
        if is_training:
            # --- BPR MODIFICATION: Only score the specific positive and negative items ---
            ...
            # --- Evaluation: Score against the target negative combinations ---
...
class DGSRLayers(nn.Module):
    def __init__(self, in_feats, out_feats, user_max_length, item_max_length, feat_drop=0.2, attn_drop=0.2, user_long='orgat', user_short='att',
        ...
        self.K = torch.tensor(K).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
    ...
    def forward(self, g, feat_dict=None):
        ...
        if feat_dict == None:
            if self.user_long in ['gcn']:
                g.nodes['user'].data['norm'] = g['by'].in_degrees().unsqueeze(1).to(device)
            if self.item_long in ['gcn']:
                g.nodes['item'].data['norm'] = g['by'].out_degrees().unsqueeze(1).to(device)
            # These two lines were missing!
            user_ = g.nodes['user'].data['user_h']
            item_ = g.nodes['item'].data['item_h']
        else:
            user_ = feat_dict['user'].to(device)
            item_ = feat_dict['item'].to(device)
            if self.user_long in ['gcn']:
                g.nodes['user'].data['norm'] = g['by'].in_degrees().unsqueeze(1).to(device)
            if self.item_long in ['gcn']:
                g.nodes['item'].data['norm'] = g['by'].out_degrees().unsqueeze(1).to(device)
...
def collate_bpr(data, user_neg_data):
    """
    BPR Dataloader collate function. 
    Samples exactly one negative item from the user's negative item list.
    """
    user_l = []
    graph = []
    label = []
    ...
    for da in data:
        # Extract user ID to look up valid negatives
        u_id = da[1]['user']
        if isinstance(u_id, torch.Tensor):
            u_id = u_id.item()
