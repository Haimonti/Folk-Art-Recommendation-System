...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # print('beta',beta)  # This will show attention weights for each feature type.
        print('beta',beta.shape)  # This will show attention weights for each feature type.
...
class HSALLayers(nn.Module):
    def __init__(self, in_feats, out_feats, user_max_length, item_max_length, feat_drop=0.2, attn_drop=0.2, user_long='orgat', user_short='att',
        ...
        self.series_table = pd.read_csv('Give path to positional embedding generated by sinusoidal.py')
    ...
    def item_update_function(self, item_now, item_old):
        print("item_now shape:", item_now.shape)
        print("item_old shape:", item_old.shape)
        # if item_old.dim() == 2:  
        #     item_old = item_old.unsqueeze(1)  # Shape: [batch_size, 1, hidden_dim]
        #     item_old = item_old.expand(-1, item_now.shape[1], -1)  # Expand across num_features

        # print("Updated item_old shape:", item_old.shape)
    ...
    def item_reduce_func(self, nodes):
        ...
        series_to_subsequent_items = {
            series_id: self.series_table[self.series_table['series_id'] == series_id]
            for series_id in self.series_table['series_id'].unique()
        }
        ...
        combined_features = torch.stack(h, dim=1)
        print('combined_features',combined_features.shape)
        fused_features = self.transformer_fusion(combined_features)
        fused_features = fused_features.view(fused_features.shape[0], -1)
        # print('fused_features',fused_features.shape)
        fused_features = self.agg_gate_i(fused_features)
        return {'item_h': fused_features}
