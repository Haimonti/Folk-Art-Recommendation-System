...
def generate_graph(data):
    ...
    graph.nodes['user'].data['user_id'] = torch.arange(num_users)
    graph.nodes['item'].data['item_id'] = torch.arange(num_items)
...
def generate_user(user, data, graph, item_max_length, user_max_length, train_path, test_path, k_hop=3, val_path=None):
    ...
    if len(u_seq) < 3:
        ...
        for j, t  in enumerate(u_time[0:-1]):
            ...
            u_temp = torch.tensor([user], dtype=torch.int32)
            his_user = torch.tensor([user])
            graph_i = select_topk(sub_graph, item_max_length, weight='time', nodes={'user':u_temp})
            i_temp = torch.unique(graph_i.edges(etype='by')[0]).to(torch.int32)
            his_item = torch.unique(graph_i.edges(etype='by')[0])
            edge_i = [graph_i.edges['by'].data[dgl.NID]]
            edge_u = []
            for _ in range(k_hop-1):
                graph_u = select_topk(sub_graph, user_max_length, weight='time', nodes={'item': i_temp.to(torch.int32)})  # item的邻居user
                ...
                graph_i = select_topk(sub_graph, item_max_length, weight='time', nodes={'user': torch.tensor(u_temp, dtype=torch.int32)})
            ...
            all_edge_u = torch.unique(torch.cat(edge_u)).to(torch.int32)
            all_edge_i = torch.unique(torch.cat(edge_i)).to(torch.int32)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', default='Goodreads_YoungAdult_HSAL_Final', help='data name: sample')
