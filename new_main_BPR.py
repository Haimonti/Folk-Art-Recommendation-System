...
import pandas as pd
import numpy as np
from DGSR_for_BPR import DGSR, collate, collate_test, collate_bpr
...
def main():
    ...
    parser.add_argument('--data', default='MAL_HSAL_Final', help='data name: sample')
    ...
    data_neg = pickle.load(f) # Used for negative samples

    # Setup BPR Partial Collate Function
    #train_collate_fn = partial(collate_bpr, user_neg_data=data_neg)

    #train_data = DataLoader(dataset=train_set, batch_size=opt.batchSize, collate_fn=train_collate_fn, shuffle=True, pin_memory=True, num_workers=12)
    ...
    # Setup Partial Collate Functions (Pickle-safe for multiprocessing)
    train_collate_fn = partial(collate_bpr, user_neg_data=data_neg)
    test_collate_fn = partial(collate_test, user_neg=data_neg)

    train_data = DataLoader(dataset=train_set, batch_size=opt.batchSize, collate_fn=train_collate_fn, shuffle=True, pin_memory=True, num_workers=12)
    test_data = DataLoader(dataset=test_set, batch_size=opt.batchSize, collate_fn=test_collate_fn, pin_memory=True, num_workers=8)
    if opt.val:
        val_data = DataLoader(dataset=val_set, batch_size=opt.batchSize, collate_fn=test_collate_fn, pin_memory=True, num_workers=2)
    ...
    for epoch in range(opt.epoch):
        ...
        for user, batch_graph, label, last_item, neg_item in train_loader:
            ...
            # Pass targeted pos_tar and neg_tar for BPR
            pos_score, neg_score = model(batch_graph, user, last_item, pos_tar=label, neg_tar=neg_item, is_training=True)
        ...
        if opt.val:
            ...
            with torch.no_grad():
                for user, batch_graph, label, last_item, neg_tar in val_loader:
                    ...
                    #val_loss = val_loss_func(score, label.cuda())
                    val_loss = val_loss_func(score, label.to(device))
                    val_loss_all.append(val_loss.item())
                    top_val.append(top.detach().cpu().numpy())
                recall5, recall10, recall20, ndgg5, ndgg10, ndgg20 = eval_metric(top_val) # Adjust according to your utils file
        ...
        with torch.no_grad():
            ...
            # Unpacking 22 metrics to match HSAL format
            recall5, recall10, recall20, recall30, recall40, recall50, recall60, recall70, recall80, recall90, recall100, \
            ndgg5, ndgg10, ndgg20, ndgg30, ndgg40, ndgg50, ndgg60, ndgg70, ndgg80, ndgg90, ndgg100 = eval_metric(all_top)
